 ...
 * kosaraju算法
 * <p>
 * 设原图为G，取反之后的图为[G],要证明kosaraju算法的正确性，我们需要证明两点：
 * 1.每个和s强连通的顶点v在dfs(G,s)中都会被访问到，证明如下：
 * 若有一个顶点v未被访问到，那么顶点v之前就被标记过了，
 * 但是如果被标记过了，那么s必定会被访问到，因为是v和s是强连通，也就不存在调用dfs(G,s)了，
 * 所以能够调用dfs(G,s)，这个强连通分量中的任意一个顶点都未被访问过。
 * 2.构造函数调用的dfs(G,s),所到达的任意顶点v必然和s是强连通的，证明如下：
